...
<body>
<%
    request.setAttribute("url","equlist.jsp");
    ...
    if(key!=null&&key!="")
    {
        where+=" and equname like '%"+key+"%'";
    }
    equmentdao dao=new equmentdao();
    List<equment> list=dao.GetList(where, Integer.parseInt(pageNo), pageSize);
        ...
        <pg:pager items="${total}" url="${url}" maxPageItems = "<%=pageSize %>" maxIndexPages="<%=pageSize %>" export="offset,currentPageNumber=pageNumber" isOffset="false"  index="center"  >
            ...
            <div class="layui-card-header">
                <form class="layui-form" action="${pageContext.request.contextPath}/admin/${url}" method="get">
                    <input type="text" placeholder="输入要查询的装备名称" value="<%=key==null?"":key%>" name="key" class="layui-input" style="display: inline;width: 200px">
                    <button class="layui-btn layui-btn-primary">提交</button>
                    <a href="${pageContext.request.contextPath}/admin/editequ.jsp" class="layui-btn layui-btn-normal">添加</a>
                </form>
            </div>
            <div class="layui-card-body">
                <table class="layui-table" lay-size="sm">
                    ...
                    <tr>
                        ...
                        <td><%=model.getEquxh() %></td>
                        <td><%=model.getEqudw() %></td>

                        <td>
                            <a href="${pageContext.request.contextPath}/admin/editequ.jsp?id=<%=model.getId() %>" class="layui-btn  layui-btn-xs"> 编辑</a> |
                            <a href="${pageContext.request.contextPath}/admin/EqumentServlet?act=del&id=<%=model.getId() %>" class="layui-btn  layui-btn-xs"> 删除</a> |
                            </td>
